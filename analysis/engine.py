...
from config import FABRIC_TYPES, PATTERN_TYPES, COLOR_TERMS, STYLE_TERMS, SEGMENTS, EUROPEAN_COUNTRIES
...
def analyze_trends(listings, google_data=None):
    ...
    style_stats = _count_term_occurrences(clean_listings, STYLE_TERMS, "style")
    ...
    _enrich_with_google(style_stats, google_data)

    # Enrich with industry trend report signals if available
    trend_signals = google_data.get("_trend_report_signals", [])
    if trend_signals:
        _enrich_with_trend_reports(
            fabric_stats + pattern_stats + color_stats + style_stats,
            trend_signals,
        )
    ...
    style_stats = _validate_and_score(style_stats)
    ...
    style_stats.sort(key=lambda x: x["score"], reverse=True)

    # Save to database
    all_snapshots = fabric_stats + pattern_stats + color_stats + style_stats
    ...
    insights = _generate_insights(
        fabric_stats, pattern_stats, color_stats, google_data, style_stats,
    )
...
def _enrich_with_google(stats, google_data):
    ...
    for item in stats:
        # Try multiple keyword formats to match
        term = item["term"]
        for key in [f"{term} fabric", f"{term} sewing", term]:
...
def _enrich_with_trend_reports(stats, trend_signals):
    """Add industry trend report signals to stats.

    Trend report data comes from authoritative sources (Pantone, fashion
    publications, textile trade shows) and gets high weight because these
    sources SET trends rather than just reflecting them.
    """
    # Build lookup
    signal_by_term = {}
    for sig in trend_signals:
        signal_by_term[sig["term"].lower()] = sig

    for item in stats:
        sig = signal_by_term.get(item["term"].lower())
        if sig:
            item["industry_signal"] = True
            item["industry_score"] = sig.get("authority_weighted_score", 0)
            item["industry_sources"] = sig.get("sources", [])
...
def _calculate_scores(stats):
    ...
    Rebalanced scoring (out of 100) — search demand and industry signals
    are weighted higher because they're leading indicators, while
    marketplace presence is a lagging indicator.

    Score components:
    - Search demand (25): Google Trends interest — leading indicator
    - Marketplace presence (20): Listings, weighted by quality — lagging
    - Momentum (15): Google trending up + acceleration signals
    - Source diversity (12): Multi-platform confirmation
    - Community validation (10): Favorites/engagement
    - Seller diversity (8): Multiple independent sellers
    - Industry signals (10): Pantone, fashion week, trend reports
    """
    if not stats:
        return
    ...
    for item in stats:
        ...
        # Search demand (25pts) — leading indicator, highest weight
        search = (item["google_interest"] / max_google) * 25

        # Marketplace presence (20pts, quality-weighted)
        presence = (item["mention_count"] / max_mentions) * 20 * quality_mult

        # Momentum (15pts) — is demand accelerating?
        momentum = 0
        if item.get("google_trending_up"):
            momentum += 10
        if item.get("google_interest", 0) > 40:
            momentum += 3
        # Pinterest presence is a visual trend signal (leading)
        pinterest_count = item.get("by_source", {}).get("pinterest", 0)
        if pinterest_count >= 3:
            momentum += 2

        # Source diversity (12pts) — multi-platform = real trend
        source_count = len(item.get("by_source", {}))
        source_div = min(source_count * 4, 12)

        # Community validation (10pts)
        validation = (item["avg_favorites"] / max_favs) * 10

        # Seller diversity (8pts)
        seller_count = item.get("unique_sellers", 1)
        seller_div = min((seller_count / max(max_sellers, 1)) * 8, 8)

        # Industry signals (10pts) — authoritative trend-setters
        industry = 0
        if item.get("industry_signal"):
            industry_score = item.get("industry_score", 0)
            industry_sources = len(item.get("industry_sources", []))
            industry = min(industry_score * 0.5 + industry_sources * 2, 10)

        raw_score = (
            search + presence + momentum + source_div +
            validation + seller_div + industry
        )
...
def _generate_insights(fabric_stats, pattern_stats, color_stats, google_data,
    ...
    all_stats = fabric_stats + pattern_stats + color_stats + style_stats
